...
router.post('/', verificarRol('ADMIN', 'CAJERO', 'MOZO'), validate({ body: crearPedidoBodySchema }), idempotency('pedidos:create'), asyncHandler(pedidosController.crear));
router.patch('/:id/estado', verificarRol('ADMIN', 'COCINERO', 'CAJERO', 'MOZO', 'DELIVERY'), validate({ params: idParamSchema, body: cambiarEstadoBodySchema }), idempotency('pedidos:update-estado'), asyncHandler(pedidosController.cambiarEstado));
...
router.post('/:id/cerrar', verificarRol('ADMIN', 'CAJERO', 'MOZO'), validate({ params: idParamSchema }), idempotency('pedidos:close'), asyncHandler(pedidosController.cerrar));
